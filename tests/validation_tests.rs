...
fn test_document_structure_validation() {
    ...
    assert_eq!(xtab.version, "1.1", "Should match expected version");
    
    // Validate that required fields are present
    assert!(xtab.date.is_some(), "Date should be present");
    assert!(xtab.time.is_some(), "Time should be present");
    assert!(xtab.user.is_some(), "User should be present");
    ...
    assert!(!xtab.control_types.is_empty(), "Should have control types");
    assert!(!xtab.statistic_types.is_empty(), "Should have statistic types");
...
fn test_control_types_validation() {
    ...
    // Should have expected control types
    let control_types = &xtab.control_types;
    assert!(control_types.len() >= 6, "Should have at least 6 control types");
    ...
    let control_type_names: Vec<&str> = control_types.iter()
        .map(|ct| ct.name.as_str())
        .collect();
    
    assert!(control_type_names.contains(&"project"), "Should have 'project' control type");
    assert!(control_type_names.contains(&"filter"), "Should have 'filter' control type");
    assert!(control_type_names.contains(&"weight"), "Should have 'weight' control type");
    assert!(control_type_names.contains(&"base"), "Should have 'base' control type");
}
...
fn test_statistic_types_validation() {
    ...
    // Should have many statistic types
    let statistic_types = &xtab.statistic_types;
    assert!(statistic_types.len() >= 70, "Should have at least 70 statistic types");
    ...
    let stat_type_names: Vec<&str> = statistic_types.iter()
        .map(|st| st.name.as_str())
        .collect();
    
    let expected_stats = [
        "Values", "Labels", "Text", "Percent", "RowPercent", "ColumnPercent",
        "TotalPercent", "Average", "Median", "Minimum", "Maximum", "Sum",
        "n", "BaseN", "StandardError", "LowerCI", "UpperCI"
    ];
    
    for expected_stat in &expected_stats {
        assert!(stat_type_names.contains(expected_stat), 
               "Should have '{}' statistic type", expected_stat);
    }
}
...
fn test_tables_validation() {
    ...
    assert!(tables.len() >= 6, "Should have at least 6 tables");
...
fn test_first_table_specific_validation() {
    ...
    // First table should be "FILTERS by CONCEPT_SEEN (X)"
    ...
    assert!(base_controls[0].text.contains("Total sample; Unweighted; base n = 624"));
    
    // Should have exactly 2 statistics: ColumnPercent and n
    assert_eq!(first_table.statistics.len(), 2);
    let stat_types: Vec<&str> = first_table.statistics.iter()
        .map(|s| s.r#type.as_str())
        .collect();
    assert!(stat_types.contains(&"ColumnPercent"));
    assert!(stat_types.contains(&"n"));
    
    // Row edge should have 16 elements
    let row_edge = first_table.row_edge.as_ref().unwrap();
    let row_elements_count: usize = row_edge.groups.iter()
        .map(|g| g.elements.len())
        .sum();
    assert_eq!(row_elements_count, 16);
    
    // Column edge should have 4 elements
    let col_edge = first_table.column_edge.as_ref().unwrap();
    let col_elements_count: usize = col_edge.groups.iter()
        .map(|g| g.elements.len())
        .sum();
    assert_eq!(col_elements_count, 4);
    
    // Data should have 16 rows
    assert_eq!(first_table.data.rows.len(), 16);
    
    // Each row should have 2 series (matching 2 statistics)
    for (row_idx, row) in first_table.data.rows.iter().enumerate() {
        assert_eq!(row.data_row_series.len(), 2, 
                  "Row {} should have 2 series", row_idx);
        
        // Each series should have 4 cells (matching 4 columns)
        for (series_idx, series) in row.data_row_series.iter().enumerate() {
            assert_eq!(series.cells.len(), 4, 
                      "Row {} series {} should have 4 cells", row_idx, series_idx);
        }
    }
}
...
fn test_table_with_summary_validation() {
    ...
    // Find a table that should have summaries (based on example structure)
    let table_with_summary = xtab.tables.iter()
        .find(|t| t.title.contains("TOP 2 BOX") || t.title.contains("TOP BOX"))
        .expect("Should find a table with summaries");
    
    // Check column edge for summaries
    if let Some(col_edge) = &table_with_summary.column_edge {
        let has_summary = col_edge.groups.iter()
            .any(|g| !g.summaries.is_empty());
        assert!(has_summary, "Table with summaries should have summary elements");
