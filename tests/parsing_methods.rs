...
fn test_parse_str_empty() {
    ...
    assert!(result.is_err(), "parse_str should fail for empty string");
...
fn test_parse_bytes_empty() {
    ...
    assert!(result.is_err(), "parse_bytes should fail for empty bytes");
...
<xtab version="1.1" xmlns:xt="http://www.XtabML.org/2005/xtab" xmlns="http://www.XtabML.org/2005/xtab">
  ...
  <table name="test-utf8">
    ...
    assert!(table.controls.iter().any(|c| c.text.contains("café naïve résumé")));
